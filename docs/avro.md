...
| `time-millis` | `int` | Строка ISO `"HH:MM:SS"` (`.fff` добавляется только если мс ≠ 0) | Строка `"HH:MM:SS[.fff]"`, полная строка `"YYYY-MM-DDTHH:MM:SS[.fff]"` (берётся только время) или число (мс с полуночи) |
| `timestamp-millis` | `long` | Строка ISO `"YYYY-MM-DDTHH:MM:SS"` (`.fff` добавляется только если мс ≠ 0, без суффикса `Z`) | Строка ISO или число (мс с эпохи UTC) |
...
- `timestamp-millis` хранит время в UTC без информации о часовом поясе. При распаковке строка возвращается **без суффикса `Z`** — он избыточен (значение всегда UTC) и мешает разбору в тип `Дата` на стороне 1С. Конвертация локального времени ↔ UTC выполняется на стороне 1С (через `УниверсальноеВремя` / `МестноеВремя`).
- Дробная часть (`.fff`) при распаковке `time-millis` / `timestamp-millis` выводится **только если миллисекунды действительно не нулевые**. Тип `Дата` в 1С имеет секундную точность и дробную часть не хранит, поэтому при нулевых мс `.000` не добавляется.
- При упаковке `time-millis` принимает не только чистую строку времени `"HH:MM:SS[.fff]"`, но и полную строку дата/время `"YYYY-MM-DDTHH:MM:SS[.fff]"` — из неё используется только временная часть. Это удобно, потому что 1С сериализует тип `Дата` целиком, добавляя дату-заглушку даже когда осмысленно только время (например, `"0001-01-01T09:00:00"` для поля «время начала»).
...
Начиная с версии 1.9.3 повторный вызов с тем же именем и **изменённым** текстом схемы применяет новую схему. Раньше схема компилировалась только при первом вызове, поэтому в переиспользуемом экземпляре компоненты (например, в фоновом задании, работающем часами) после эволюции схемы продолжала действовать первая версия — без сообщения об ошибке. Если текст схемы не изменился, метод выходит сразу и повторной компиляции не выполняет, поэтому вызывать его перед каждым сообщением безопасно.

#### Ветки `union`

Ветка union подбирается по схеме и типу значения JSON, а не по позиции. Поддерживаются союзы, где `null` стоит не первым (`["string", "null"]`), союзы из трёх и более веток и ссылки на именованный тип в рекурсивных схемах. Поле, отсутствующее в JSON, получает ветку `null`, если она есть в схеме.
...
```1c
ДекодироватьСообщениеAVRO(ДанныеAVRO, ИмяСхемыJSON, ВозвращатьJSON, ЭтоBase64)
```
...
**ЭтоBase64** (`IsBase64`) - Булево. Необязательный. По умолчанию **Ложь**:
- **Ложь** - байты берутся как есть. Это прежнее поведение, существующий код 1С менять не требуется
- **Истина** - вход сначала декодируется из base64, и только результат разбирается как AVRO.
  Если вход не является корректным base64, метод возвращает пустую строку и записывает ошибку;
  тихого возврата к сырым байтам не происходит

Подробнее о том, когда нужен этот параметр — [Данные приходят как base64-текст](#данные-приходят-как-base64-текст).
...
#### Данные приходят как base64-текст

1С в ряде случаев передаёт `ДвоичныеДанные` во внешнюю компоненту не сырыми байтами, а
**base64-текстом**. Компонента в этом случае видит на входе не AVRO, а строку вида
`T2JqAQQWYXZyby5zY2hlbWG...`, и декодирование ожидаемо падает с ошибкой формата — при этом сами
данные корректны, испорчен только способ их передачи.

Признаки: метод возвращает пустую строку, `ПолучитьСообщениеОбОшибке()` сообщает о некорректном
AVRO, а те же данные, записанные в файл и открытые сторонним инструментом, читаются нормально.

Решение — передать `ЭтоBase64 = Истина`:

```1С
// Данные, которые 1С отдала компоненте в виде base64-текста
ДанныеAVRO = ПолучитьДанныеИзВнешнегоИсточника();

// Четвёртый параметр говорит компоненте сначала раскодировать base64
ДанныеJSON = Компонента.ДекодироватьСообщениеAVRO(ДанныеAVRO, "User", Истина, Истина);

Если ПустаяСтрока(ДанныеJSON) Тогда
    // Сюда попадаем и если вход не является корректным base64 —
    // компонента сообщит об этом явно, а не попытается разобрать байты как есть
    ВызватьИсключение Компонента.ПолучитьСообщениеОбОшибке();
КонецЕсли;
...
**Почему параметр нужно указывать явно, а не определять автоматически.** Сырой AVRO-payload
может целиком состоять из символов алфавита base64 — такие данные неотличимы от base64-текста.
Автоматическое определение в этом случае молча раскодировало бы корректное сообщение в мусор,
поэтому режим задаёт только вызывающий код: он единственный точно знает, что именно передал.

По той же причине при `ЭтоBase64 = Истина` некорректный base64 считается ошибкой, а не поводом
вернуться к сырым байтам. Проверка строгая: посторонние символы, данные после символов `=`,
больше двух символов `=` и паддинг, не дополняющий группу до кратной четырём, отвергаются.
Допускаются оба алфавита (`+/` и URL-safe `-_`), полностью опущенный паддинг,
а также пробелы и переносы строк в любом месте.
...
```1c
ПолучитьСхемуAVRO(ДанныеAVRO, ЭтоBase64)
```

**ДанныеAVRO** - БинарныеДанные. Обязательный. AVRO данные в формате OCF (Object Container File).

**ЭтоBase64** (`IsBase64`) - Булево. Необязательный. По умолчанию **Ложь**. Работает так же, как
одноимённый параметр метода **ДекодироватьСообщениеAVRO** — см.
[Данные приходят как base64-текст](#данные-приходят-как-base64-текст). При **Истина** вход сначала
раскодируется из base64, и некорректный base64 даёт ошибку, а не разбор сырых байт.
...
#### Пример: данные переданы как base64-текст

```1С
// ДанныеAVRO пришли в виде base64-строки, а не сырых байт
Схема = Компонента.ПолучитьСхемуAVRO(ДанныеAVRO, Истина);

Если ПустаяСтрока(Схема) Тогда
    ВызватьИсключение Компонента.ПолучитьСообщениеОбОшибке();
КонецЕсли;

// Те же данные декодируем тем же режимом
ДанныеJSON = Компонента.ДекодироватьСообщениеAVRO(ДанныеAVRO, "", Истина, Истина);
```
