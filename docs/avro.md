...
**Avro OCF** — формат, создаваемый методом `ПреобразоватьВФорматAVRO`. Содержит схему в заголовке файла.

**Confluent Wire Format** — формат, используемый Confluent Schema Registry. Первый байт = `0x00`, затем 4 байта schema ID, затем raw Avro данные. Этот формат автоматически распознаётся, и 5-байтовый заголовок пропускается.

**Raw Avro** — просто сериализованные Avro данные без контейнера. Часто используется при передаче через Kafka от внешних систем.
