...
| [ЧитатьПакетСообщений](#пакетное-чтение-сообщений) | Пакетное чтение нескольких сообщений |
...
| [ПолучитьМетрикиКонсьюмера](#метрики-консьюмера) | Получение статистики работы консьюмера |
| [УстановитьТаймаутОчисткиКонсьюмера](#настройка-таймаута-остановки) | Установка таймаута для остановки консьюмера |
...
```1c
УстановитьПозициюЧтения(Топик, Смещение, Партиция, Брокер);
```

**Топик** - Строка. Имя топика.

**Смещение** - Число. Позиция, с которой нужно начать чтение. Принимается 64-битное значение
(точно — до 2^53). Отрицательные значения сохраняют прежний смысл специальных смещений librdkafka
(`-1` — конец, `-2` — начало, `-1000` — сохранённое) и передаются без изменений.

**Партиция** - Число. Номер партиции, по умолчанию = 0.

**Брокер** - Строка, **необязательный** (по умолчанию `""`). Адрес брокеров для проверки существования топика/партиции. Если задан — используется для валидации; если не задан — адрес берётся из настроек, а при их отсутствии проверка существования топика пропускается. Подробнее см. раздел [Валидация устанавливаемых позиций](#валидация-устанавливаемых-позиций).
...
> **Важно:** метод выполняет валидацию и может вернуть `Ложь`. Подробнее см. раздел [Валидация устанавливаемых позиций](#валидация-устанавливаемых-позиций) ниже.

```1c
УстановитьПозицииЧтения(СтрокаJSON, Брокер);
```
...
**Брокер** - Строка, **необязательный** (по умолчанию `""`). Адрес брокеров для проверки существования топиков/партиций. Если задан — используется для валидации; если не задан — адрес берётся из настроек, а при их отсутствии проверка существования топика пропускается. Подробнее см. раздел [Валидация устанавливаемых позиций](#валидация-устанавливаемых-позиций).
...
### Валидация устанавливаемых позиций

Оба метода — `УстановитьПозициюЧтения` и `УстановитьПозицииЧтения` — выполняют проверку входных данных и при ошибке возвращают `Ложь` (ранее всегда возвращалось `Истина`). Текст ошибки доступен через `ПолучитьСообщениеОбОшибке()`.

Проверки делятся на две группы.

**1. Валидация аргументов** (выполняется всегда):
- имя топика не пустое и соответствует правилам именования Kafka;
- номер партиции `>= 0`;
- корректный тип переданных аргументов;
- для `УстановитьПозицииЧтения` дополнительно — корректный JSON, наличие массива `metadata` и непустой массив.

**2. Проверка существования топика и партиции** (выполняется, **только если** доступен адрес брокеров):
- через временный клиент запрашиваются метаданные кластера; если топик или указанная партиция не существуют — возвращается `Ложь`;
- для **неотрицательного** смещения дополнительно проверяется, что оно не превышает верхнюю границу партиции (high watermark). Специальные отрицательные смещения (начало/конец/сохранённое) проверке диапазона не подвергаются.

Адрес брокеров определяется в следующем порядке приоритета:
1. **Необязательный параметр `Брокер`** метода (если передан непустой строкой);
2. иначе — настройки: `bootstrap.servers`, затем `metadata.broker.list` (заданные через `УстановитьПараметр`).

> **Важно:** если адрес брокеров недоступен ни одним из способов (например, он передаётся только в `ИнициализироватьКонсьюмера`, которая вызывается позже), проверка существования топика **пропускается** — выполняется только валидация аргументов. Чтобы включить полноценную проверку на этапе установки позиции, передайте адрес параметром `Брокер` либо задайте его настройкой до вызова метода.

> Метод `УстановитьПозицииЧтения` применяет позиции **атомарно**: если хотя бы одна запись не прошла проверку, не применяется ни одна из них.

Пример с проверкой существования топика через параметр `Брокер`:
...
// Передаём адрес брокеров параметром Брокер — метод проверит существование топика/партиции
Если Не Компонента.УстановитьПозицииЧтения("{""metadata"":[{""topic"":""my-topic"",""partition"":0,""offset"":41}]}", "localhost:9092") Тогда
    Сообщить("Ошибка установки позиции: " + Компонента.ПолучитьСообщениеОбОшибке());
    Возврат;
КонецЕсли;
...
**ВременнаяМетка** - Строка или Число. Unix timestamp в миллисекундах, целое неотрицательное число.
Значение передается целиком: делить его на 1000 не нужно — секунды методу не подходят, и дробное значение
будет отвергнуто с ошибкой.

**Передавайте метку Строкой:** `"1788972117044"`. Число в миллисекундах не помещается в 32 бита, а платформа
1С в ряде версий передает такие значения во внешнюю компоненту, обрезав до 32 бит — в компоненту приходит
испорченное число, и переход выполняется не туда. Строка проходит без потерь на любой платформе.

Число тоже принимается, но метка, пришедшая в компоненту как 32-битное целое, отвергается с ошибкой:
восстановить исходное значение невозможно, а тихий переход к неверной позиции хуже явного отказа.
Текст ошибки предлагает передать значение Строкой.
...
ВременнаяМетка = Формат((ДатаНачала - Дата(1970, 1, 1)) * 1000, "ЧГ=0"); // миллисекунды, Строкой
...
Тексты ошибок (`ПолучитьСообщениеОбОшибке()`):

| Причина | Сообщение |
|---|---|
| метка пришла обрезанной до 32 бит | `seekToTimestamp: timestamp - the number arrived from 1C as int32 and no longer holds a millisecond timestamp. Pass the timestamp as a String, for example "1788972117044"` |
| метка не число и не строка | `seekToTimestamp: timestamp - expected a number` |
| в строке не только цифры | `seekToTimestamp: timestamp - expected a whole number of milliseconds since epoch, got "1 788 972 117 044"` |
| метка дробная | `seekToTimestamp: timestamp - expected a whole number, got 1.78897e+09` |
| метка больше 2^53 | `seekToTimestamp: timestamp - number exceeds the exact integer range (2^53)` |
| метка отрицательная | `seekToTimestamp: timestamp - timestamp must be a non-negative number of milliseconds since epoch` |
| в партиции нет подходящего сообщения | `No offset found for the specified timestamp: the partition has no message with a timestamp at or after the given one` |
...
> - Партиция должна быть назначена консьюмеру (через `Подписаться` или `НазначитьПартиции`),
>   иначе переход завершится ошибкой состояния
> - Метку передавайте Строкой: `Формат(Метка, "ЧГ=0")`. Без `"ЧГ=0"` в строку попадут разделители групп
>   разрядов, и значение будет отвергнуто
...
> **Разрядность смещения.** Смещение в Kafka — 64-битное число. Ранее геттер сужал его до 32 бит,
> поэтому на топиках со смещением выше 2 147 483 647 возвращалось отрицательное или неверное значение.
> Теперь значение отдаётся без сужения. Смещение передаётся в 1С как число с плавающей точкой,
> целые значения представляются точно до 9 007 199 254 740 992 (2^53) — предел, недостижимый
> для реальных топиков Kafka.
...
Для эффективного чтения большого количества сообщений за один вызов используется метод **ЧитатьПакетСообщений**.

```1c
ЧитатьПакетСообщений(МаксимальноеКоличество, МаксимальноеВремяОжидания, КодироватьВBase64)
```

**МаксимальноеКоличество** - Число. Максимальное количество сообщений для чтения. По умолчанию 100.

**МаксимальноеВремяОжидания** - Число. Максимальное время ожидания сообщений в миллисекундах. По умолчанию 1000.

**КодироватьВBase64** - Булево. Если `Истина`, поле `message` возвращается в Base64 (рекомендуется для бинарных данных). По умолчанию `Ложь`.

Метод возвращает строку JSON с массивом сообщений.

Структура:
+ messages - Массив сообщений
  + message - Тело сообщения (строка, либо Base64 при `КодироватьВBase64=Истина`)
  + message_encoding - `"base64"` (добавляется, если `КодироватьВBase64=Ложь`, но сообщение не в UTF-8)
  + key - Ключ сообщения (если есть)
  + key_encoding - `"base64"` (добавляется, если ключ не в UTF-8)
  + topic - Топик
  + partition - Номер партиции
  + offset - Смещение
  + timestamp - Временная метка (ms)
  + broker_id - ID брокера
  + headers - Массив заголовков (если есть)
+ count - Количество прочитанных сообщений

Особенности поведения:

1. Если `payload` сообщения пустой (`null` или длина 0), в `message` возвращается пустая строка.
2. Если значение заголовка (`headers[i].value`) отсутствует (`null`), возвращается пустая строка.
   Если значение не в UTF-8, оно возвращается в Base64 и добавляется `headers[i].encoding = "base64"`.
3. Метод может вернуть JSON с `count = 0`, если за время ожидания сообщений не было.
4. Если `КодироватьВBase64 = Ложь`, но `message`/`key` содержат бинарные данные (не UTF-8), они автоматически возвращаются в Base64, чтобы сообщение не терялось при формировании JSON.
...
Компонента.УстановитьПараметр("group.id", "my-batch-group");
Компонента.ИнициализироватьКонсьюмера("localhost:9092");
Компонента.Подписаться("my-topic");

// Читаем до 50 сообщений с ожиданием до 2 секунд.
// Третий параметр = Ложь: вернуть message как обычную строку.
РезультатJSON = Компонента.ЧитатьПакетСообщений(50, 2000, Ложь);
...
Сообщить("Получено сообщений: " + Результат["count"]);

Для Каждого Сообщение Из Результат["messages"] Цикл
    Сообщить("Топик: " + Сообщение["topic"] +
             ", Партиция: " + Сообщение["partition"] +
             ", Offset: " + Сообщение["offset"]);
    Сообщить("Данные: " + Сообщение["message"]);
КонецЦикла;
...
```json
{
    "messages": [
        {
            "message": "Содержимое сообщения 1",
            "key": "key-001",
            "topic": "my-topic",
            "partition": 0,
            "offset": 1500,
        ...
        {
            "message": "Содержимое сообщения 2",
            "topic": "my-topic",
            "partition": 0,
            "offset": 1501,
...
**Когда использовать пакетное чтение:**

1. **Высокая пропускная способность** - когда нужно обработать большой поток сообщений.

2. **Пакетная обработка** - когда бизнес-логика требует обработки данных пакетами (например, bulk-insert в базу данных).

3. **Снижение накладных расходов** - меньше вызовов API и парсинга JSON.

> **Примечание:** Метод может вернуть меньше сообщений, чем запрошено, если в очереди их недостаточно или истекло время ожидания.
...
**Смещение** - Число. Значение смещения прочитанного сообщения + 1. Принимается 64-битное значение
(точно — до 2^53), отрицательные значения отвергаются.
...
Проверки входных данных и соответствующие тексты ошибок:

| Ситуация | Текст ошибки |
|----------|--------------|
| консьюмер не инициализирован | `Consumer not initialized` |
| смещение не является числом | `commitOffset: offset - expected a number` |
| смещение дробное | `commitOffset: offset - expected a whole number, got 100.5` |
| смещение больше 2^53 | `commitOffset: offset - number exceeds the exact integer range (2^53)` |
| смещение отрицательное | `Offset must be >= 0` |
| номер партиции отрицательный | `Partition must be >= 0` |

> **Специальные позиции.** Отрицательное смещение здесь не имеет смысла: переход к началу, к концу
> или к временной метке выполняется методами `ПерейтиКНачалу`, `ПерейтиККонцу` и
> `ПерейтиКВременнойМетке`, а не фиксацией смещения.

### ПрочитатьСообщениеПоСмещению

Метод позволяет прочитать **одно конкретное сообщение** по точному адресу `(топик, партиция, смещение)` без влияния на основной консьюмер. Удобно для отладки, перечитывания конкретного сообщения из производственной системы или построения служебных утилит.

```1c
ПрочитатьСообщениеПоСмещению(Брокер, ИмяТопика, Партиция, Смещение, Таймаут = 10000)
```
...
**Смещение** - Число. Точное смещение сообщения, которое нужно прочитать. Принимается 64-битное
значение (точно — до 2^53), отрицательные значения отвергаются с ошибкой `Offset must be >= 0`.

**Таймаут** - Число. Таймаут ожидания сообщения в миллисекундах. По умолчанию 10000.

Метод создаёт временный `KafkaConsumer` с уникальным synthetic `group.id` (не повлияет на существующие группы), выполняет `assign` на указанную партицию с заданным смещением, читает одно сообщение и закрывается. Параметры SSL/SASL, установленные через `УстановитьПараметр`, применяются автоматически.

Метод возвращает строку JSON с данными сообщения.

Структура ответа:
+ message - Содержимое сообщения (если не UTF-8 - закодировано в base64, добавляется `message_encoding: "base64"`)
+ key - Ключ сообщения (аналогичная логика кодирования)
+ topic - Имя топика
+ partition - Номер партиции
+ offset - Смещение сообщения
+ broker_id - Идентификатор брокера
+ timestamp - Временная метка сообщения
+ headers - Массив заголовков (опционально)
...
// Прочитать сообщение из топика my-topic, партиция 0, смещение 1234
РезультатJSON = Компонента.ПрочитатьСообщениеПоСмещению("localhost:9092", "my-topic", 0, 1234);
...
> **Примечание:** Если по указанному смещению нет сообщения (например, оно было удалено retention policy или offset за пределами диапазона), метод вернёт пустую строку, а в `ПолучитьСообщениеОбОшибке()` будет сообщение вида `"No message at offset N (end of partition)"` или `"Timeout waiting for message at offset N"`.
...
```1c
ПолучитьМетрикиКонсьюмера()
```

Метод возвращает строку JSON с метриками.

Структура:
+ messages_consumed - Количество прочитанных сообщений
+ bytes_consumed - Объем прочитанных данных в байтах
+ errors_count - Количество ошибок при чтении
+ poll_timeouts - Количество таймаутов ожидания (пустых poll)
+ uptime_seconds - Время работы консьюмера в секундах
...
// Читаем сообщения
Счетчик = 0;
Пока Компонента.ПрочитатьСообщение() И Счетчик < 100 Цикл
    Данные = Компонента.ПолучитьДанныеСообщения(Ложь);
    // Обработка данных...
    Счетчик = Счетчик + 1;
КонецЦикла;
...
// Получаем метрики
МетрикиJSON = Компонента.ПолучитьМетрикиКонсьюмера();
...
Сообщить("Прочитано сообщений: " + Метрики["messages_consumed"]);
Сообщить("Прочитано байт: " + Метрики["bytes_consumed"]);
Сообщить("Ошибок: " + Метрики["errors_count"]);
Сообщить("Пустых poll: " + Метрики["poll_timeouts"]);
...
```json
{
    ...
    "bytes_consumed": "45000",
    "errors_count": "0",
    "poll_timeouts": "5",
    "uptime_seconds": "30.125"
}
```

**Практические применения:**

1. **Мониторинг производительности** - отслеживание скорости чтения сообщений.

2. **Диагностика проблем** - высокое значение poll_timeouts может указывать на отсутствие новых сообщений или проблемы с подключением.

3. **Расчет пропускной способности** - bytes_consumed / uptime_seconds дает среднюю скорость в байтах/сек.

---

### Настройка таймаута остановки

> **Устарел с версии 1.8.8 — метод сохранён только для обратной совместимости и больше не влияет на поведение `ОстановитьКонсьюмера`.**
>
> Ранее значение использовалось для глобального `RdKafka::wait_destroyed`, ожидающего завершения **всех** фоновых потоков librdkafka в процессе. Этот вызов был удалён в версии 1.8.8: он не относится к остановке конкретного консьюмера и почти всегда приводил к ложно-отрицательному результату `ОстановитьКонсьюмера` (см. CHANGELOG). Метод оставлен как no-op, чтобы существующий 1С-код не падал.

Сигнатура (без изменений):

```1c
УстановитьТаймаутОчисткиКонсьюмера(Таймаут)
```

**Таймаут** - Число. Таймаут в миллисекундах. Значение сохраняется во внутреннем поле, но нигде не читается.

**Возвращаемое значение:** Булево. Всегда `Истина`.

Рекомендация: удалить вызов из кода 1С. В будущих мажорных версиях метод может быть полностью убран.

---

### Завершение работы консьюмера

После завершения чтения сообщений необходимо обязательно остановить консьюмера для корректного освобождения ресурсов.
...
- `Истина` - консьюмер успешно остановлен, все ресурсы освобождены (или консьюмер не был инициализирован).
- `Ложь` - произошла ошибка при остановке консьюмера.

**Когда возвращается `Ложь`:**
1. **Ошибка close** - не удалось корректно закрыть подключение консьюмера (выход из группы, финальные коммиты и т.п.). Текст ошибки можно получить методом `ПолучитьСообщениеОбОшибке()`.

**Что делает метод:**
1. Закрывает подключение консьюмера (`close`) и проверяет результат
2. Освобождает ресурсы консьюмера
3. Очищает handle консьюмера

> **Изменение в 1.8.8:** ранее метод дополнительно вызывал глобальный `RdKafka::wait_destroyed`, который ждёт завершения **всех** фоновых потоков librdkafka в процессе и почти всегда возвращал таймаут (-1), из-за чего `ОстановитьКонсьюмера` выдавал ложную ошибку «Consumer wait_destroyed timeout». Этот вызов удалён.

Метод строго **обязательный**. Несвоевременная остановка консьюмера может привести к утечке ресурсов и проблемам с ребалансировкой в группе консьюмеров.
...
// Остановка консьюмера (обязательно!)
Если Компонента.ОстановитьКонсьюмера() Тогда
    Сообщить("Консьюмер успешно остановлен");
Иначе
    // Произошла ошибка при остановке
    ОписаниеОшибки = Компонента.ПолучитьСообщениеОбОшибке();
    ЗаписьЖурналаРегистрации("Kafka.Consumer", УровеньЖурналаРегистрации.Предупреждение,,,
        "Ошибка остановки консьюмера: " + ОписаниеОшибки);
КонецЕсли;

Компонента = Неопределено;
```

> **Рекомендация:** Если метод вернул `Ложь`, консьюмер всё равно был освобождён, но могли возникнуть проблемы:
> 1. Не все офсеты были зафиксированы (при `enable.auto.commit=true`)
> 2. Консьюмер некорректно покинул группу, что может вызвать задержку ребалансировки
> 3. Остались незавершённые фоновые операции 
