...
- `Истина` - таймаут успешно установлен.
...
// Устанавливаем таймаут остановки 30 секунд
Компонента.УстановитьТаймаутОчисткиКонсьюмера(30000);
...
// При остановке консьюмер будет ждать до 30 секунд для завершения операций
...
> **Примечание:** Увеличьте таймаут, если консьюмер работает в большой группе или на медленной сети, где ребалансировка и фиксация офсетов могут занять больше времени.
...
1. **Ошибка close** - не удалось корректно закрыть подключение консьюмера.
2. **Таймаут ожидания** - фоновые операции не завершились за отведённое время (`wait_destroyed` timeout). Таймаут по умолчанию 10 секунд, настраивается через `УстановитьТаймаутОчисткиКонсьюмера()`.

В случае возврата `Ложь` текст ошибки можно получить методом `ПолучитьСообщениеОбОшибке()`.

**Что делает метод:**
1. Закрывает подключение консьюмера (`close`) и проверяет результат
2. Освобождает ресурсы консьюмера
3. Ожидает завершения всех фоновых операций (таймаут настраивается через `УстановитьТаймаутОчисткиКонсьюмера`)
4. Очищает handle консьюмера
