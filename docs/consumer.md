...
**ВременнаяМетка** - Число. Unix timestamp в миллисекундах, целое неотрицательное число.
Значение передается целиком: делить его на 1000 не нужно — секунды метода не подходят, и дробное значение
будет отвергнуто с ошибкой. Метки больше 2 147 483 647 (то есть любые метки в миллисекундах) поддерживаются.
