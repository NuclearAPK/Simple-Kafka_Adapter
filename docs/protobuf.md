...
Начиная с версии 1.9.3 повторный вызов с тем же именем и **изменённым** текстом схемы применяет новую схему: контекст со всеми известными схемами пересобирается заново. Раньше метод при найденном имени просто возвращал "Истина", поэтому в переиспользуемом экземпляре компоненты (например, в фоновом задании, работающем часами) после эволюции схемы продолжала действовать первая версия — без сообщения об ошибке. Если текст схемы не изменился, метод выходит сразу, поэтому вызывать его перед каждым сообщением безопасно.
