...
Метод ничего не возвращает. Если имя или значение параметра передано не строкой, параметр не сохраняется, а причина записывается в текст ошибки — она доступна через **ПолучитьСообщениеОбОшибке** (раньше в 1С прилетало исключение C++ `bad_variant_access` без указания метода и параметра). По той же причине метод **Отправить** возвращает `-1` с текстом ошибки, если имя топика, ключ или заголовки переданы не строкой, а номер партиции — не числом; тело сообщения по-прежнему допускается и строкой, и двоичными данными.

Начиная с версии 1.9.3 повторный вызов с уже установленным именем **заменяет** значение, а не добавляет вторую запись. Раньше в списке настроек оставались обе пары: при применении конфигурации побеждала последняя, а идентификатор клиента (`client.id`) для логов брался из первой, поэтому логи расходились с реально применёнными настройками. Это же правило действует для метода **УстановитьПартиционирование**, который хранит значение в параметре `partitioner`.

```1c
Компонента.УстановитьПараметр("client.id", "worker-1");
Компонента.УстановитьПараметр("client.id", "worker-2"); // осталось одно значение: worker-2
```
...
#### Причина отказа доставки

При вердикте **0** текст ошибки содержит причину, полученную из отчёта о доставке (delivery report):
код ошибки librdkafka, топик и партицию. Обращаться к логу продюсера для выяснения причины больше
не требуется.

Примеры текста, возвращаемого методом `ПолучитьСообщениеОбОшибке`:

| Ситуация | Текст ошибки |
|----------|--------------|
| топик запрещён политикой ACL | `Delivery failed: Broker: Topic authorization failed (topic=my-topic, partition=-1, code=29)` |
| топик не существует, автосоздание выключено | `Delivery failed: Broker: Unknown topic or partition (topic=my-topic, partition=-1, code=3)` |
| истёк `message.timeout.ms` | `Delivery failed: Local: Message timed out (topic=my-topic, partition=0, code=-192)` |
| сообщения остались в очереди после flush | `Messages not delivered: 1 - Delivery failed: ...` |
| отчёт о доставке так и не пришёл | `Not delivered: no delivery report received` |

Причина относится именно к текущей отправке: перед каждым вызовом метода сохранённая ошибка
предыдущей отправки сбрасывается.

Пример разбора на стороне 1С:

```1c
РезультатОтправки = Компонента.ОтправитьСообщениеСОжиданиемРезультата(Сообщение, Топик, , Ключ);

Если РезультатОтправки = 2 Тогда
    // Доставлено и подтверждено брокером
ИначеЕсли РезультатОтправки = 1 Тогда
    ЗаписьЖурналаРегистрации("Kafka", УровеньЖурналаРегистрации.Предупреждение, , ,
        "Доставка не подтверждена: " + Компонента.ПолучитьСообщениеОбОшибке());
Иначе
    // 0 или -1 - сообщение не доставлено, причина доступна в тексте ошибки
    ЗаписьЖурналаРегистрации("Kafka", УровеньЖурналаРегистрации.Ошибка, , ,
        "Сообщение не доставлено: " + Компонента.ПолучитьСообщениеОбОшибке());
КонецЕсли;
```
...
## Пакетная отправка с детальным результатом

### ОтправитьПакетСРезультатом

```1c
ОтправитьПакетСРезультатом(СообщенияJSON, ИмяТопика, ТаймаутFlush = 20000)
```

В отличие от `ОтправитьПакетСообщений`, который возвращает только число успешно отправленных сообщений, этот метод выполняет `flush` после отправки всех сообщений и возвращает **детальный результат по каждому сообщению**: подтверждённые смещение и партиция, статус, а также текст ошибки для недоставленных сообщений.

**СообщенияJSON** - Строка. Массив сообщений в том же формате, что и для `ОтправитьПакетСообщений` (см. выше).

**ИмяТопика** - Строка. Имя топика для отправки всех сообщений.

**ТаймаутFlush** - Число. Таймаут ожидания подтверждения доставки для всех сообщений в миллисекундах. По умолчанию 20000.

Метод возвращает строку JSON с результатами отправки.

Структура ответа:
+ total - Общее количество сообщений в пакете
+ success_count - Количество успешно доставленных
+ failed_count - Количество недоставленных
+ results - Массив результатов по каждому сообщению
  + index - Индекс сообщения в исходном массиве (с 0)
  + delivered - Признак успешной доставки (true/false)
  + partition - Партиция, в которую было записано сообщение (-1, если не доставлено)
  + offset - Присвоенный смещение (-1, если не доставлено)
  + status - Статус доставки librdkafka (0=NotPersisted, 1=PossiblyPersisted, 2=Persisted)
  + key - Ключ сообщения (если был задан)
  + error - Текст ошибки для недоставленных сообщений (пустая строка для успешных)
...
РезультатJSON = Компонента.ОтправитьПакетСРезультатом(СообщенияJSON, "my-topic");
...
```json
{
    ...
    "failed_count": "0",
    "results": [
        { "index": "0", "delivered": "true", "partition": "0", "offset": "42", "status": "2", "key": "k1", "error": "" },
        { "index": "1", "delivered": "true", "partition": "1", "offset": "15", "status": "2", "key": "k2", "error": "" },
        { "index": "2", "delivered": "true", "partition": "0", "offset": "43", "status": "2", "key": "k3", "error": "" }
    ]
}
```

> **Примечание:** Метод блокируется до завершения `flush` или истечения `ТаймаутFlush`. Для очень больших пакетов рекомендуется увеличить таймаут.
...
- `Истина` - продюсер успешно остановлен, все сообщения из очереди доставлены (или продюсер не был инициализирован).
- `Ложь` - произошла ошибка при остановке продюсера.

**Когда возвращается `Ложь`:**
1. **Таймаут flush** - не все сообщения были доставлены за отведённое время (см. `УстановитьТаймаутОчисткиПродюсера`).
2. **Ошибка flush** - произошла ошибка при очистке очереди сообщений.

В случае возврата `Ложь` текст ошибки можно получить методом `ПолучитьСообщениеОбОшибке()`.

**Что делает метод:**
1. Выполняет `flush` - ожидает доставки всех сообщений из внутренней очереди
2. Проверяет, что все сообщения доставлены (`outq_len() == 0`)
3. Освобождает ресурсы продюсера
4. Очищает handle продюсера
...
// Увеличиваем таймаут для большого объёма сообщений
Компонента.УстановитьТаймаутОчисткиПродюсера(30000); // 30 секунд
...
Иначе
    ...
    ЗаписьЖурналаРегистрации("Kafka.Producer", УровеньЖурналаРегистрации.Ошибка,,,
        "Ошибка остановки продюсера: " + ОписаниеОшибки);
КонецЕсли;

Компонента = Неопределено;
```

> **Рекомендация:** Если метод вернул `Ложь` с ошибкой таймаута, это означает, что часть сообщений могла не быть доставлена. Рекомендуется:
> 1. Увеличить таймаут через `УстановитьТаймаутОчисткиПродюсера()`
> 2. Проверить доступность брокеров
> 3. Использовать синхронную отправку `ОтправитьСообщениеСОжиданиемРезультата()` для критичных сообщений
