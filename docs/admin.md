...
```1c
ПолучитьСписокТопиков(Брокер, Таймаут = 5000)
```

**Брокер** - Строка. Брокер, для которого необходимо получить список топиков.

**Таймаут** - Число. Таймаут ожидания метаданных от брокера в миллисекундах. Необязательный, по умолчанию 5000.
До версии 1.9.5 таймаут был зашит в коде и составлял 5000 мс без возможности изменения.
...
При недоступности брокера метод возвращает пустую строку, а текст ошибки доступен через `ПолучитьСообщениеОбОшибке()`.
Для кластеров с большим количеством топиков или медленным каналом связи значение таймаута стоит увеличить.
...
// Значение таймаута по умолчанию - 5000 мс
СписокТопиков = Компонента.ПолучитьСписокТопиков("localhost:9092");

// Увеличенный таймаут для медленного кластера
СписокТопиков = Компонента.ПолучитьСписокТопиков("localhost:9092", 15000);

Если ПустаяСтрока(СписокТопиков) Тогда
    Сообщить("Ошибка получения списка топиков: " + Компонента.ПолучитьСообщениеОбОшибке());
Иначе
    ЧтениеJSON = Новый ЧтениеJSON;
    ЧтениеJSON.УстановитьСтроку(СписокТопиков);
    Результат = ПрочитатьJSON(ЧтениеJSON);
    ЧтениеJSON.Закрыть();

    Для Каждого Топик Из Результат.topics Цикл
        Сообщить(Топик.topic + " - партиций: " + Топик.partitions);
    КонецЦикла;
КонецЕсли;
```
...
Удаляет указанную группу консьюмеров вместе с её сохранёнными офсетами.
...
> **Важно:**
> - Группа должна быть неактивной (без активных консьюмеров), иначе брокер вернёт ошибку
>   `NON_EMPTY_GROUP`. Перед вызовом остановите консьюмеров и дождитесь `member_count = 0`
>   через **ОписатьГруппуКонсьюмеров**
> - Удаляется сама группа вместе с её офсетами; группа создаётся заново при следующем
>   подключении консьюмера с тем же идентификатором
...
**ПозицияСброса** - Строка или Число. Позиция для сброса офсетов:
- `"earliest"` - начало партиции (low watermark, запрашивается у брокера)
- `"latest"` - конец партиции (high watermark, запрашивается у брокера)
- Числовое значение - timestamp в миллисекундах; для каждой партиции берется первое сообщение
  с меткой не раньше указанной. **Передавайте метку Строкой** (`"1704067200000"`): метка в миллисекундах
  не помещается в 32 бита, а платформа 1С в ряде версий обрезает такие Числа при передаче во внешнюю
  компоненту. Число тоже принимается, но метка, пришедшая в компоненту как 32-битное целое, отвергается
  с ошибкой — вместо тихого сброса офсетов на неверную позицию.
  Если в партиции нет сообщений с меткой не раньше указанной, ее офсет ставится на конец партиции —
  так же ведет себя штатная утилита `kafka-consumer-groups --reset-offsets --to-datetime`.

Офсеты фиксируются сразу для всех партиций топика.
...
>
> Офсеты фиксирует сам инициализированный консьюмер, поэтому сбросить можно только его собственную группу:
> значение параметра **ИдентификаторГруппы** должно совпадать с параметром `group.id`, установленным через
> `УстановитьПараметр` до вызова `ИнициализироватьКонсьюмера`. При расхождении метод возвращает `Ложь`
> с описанием несоответствия. Параметр **Брокер** проверяется на корректность формата; подключение
> используется то, с которым инициализирован консьюмер.
