...
Параметр извлекался как `int64_t tTimestamp = std::get<int32_t>(timestamp)`. Unix timestamp в миллисекундах (например, `1788879414664`) в `int32` не помещается, поэтому 1С маршалит такое число как `VTYPE_R8`, и в `variant_t` оно приходит как `double`. `std::get<int32_t>` на этом варианте бросал `std::bad_variant_access`: метод возвращал `Ложь` с текстом `Exception in seekToTimestamp: bad variant access`. Обходной путь с делением на 1000 давал ту же ошибку — дробное значение тоже приходит как `double`, а метод по документации ожидает миллисекунды. Теперь параметр читается через `variantToInt64()` (принимает `int32` и `double`, отвергает дробные значения, NaN и величины свыше 2^53), добавлены проверки имени топика, номера партиции и знака метки. Дополнительно разбирается ошибка отдельной партиции (`TopicPartition::err()`): `offsetsForTimes` возвращает часть ошибок именно так, и раньше они терялись за общим кодом возврата.
